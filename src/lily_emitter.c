...
static void finish_block_code(lily_emit_state *emit)
{
    ...
    if ((block->flags & BLOCK_MAKE_CLOSURE) == 0) {
        ...
        lily_cl_transform(emit, block, f);
...
/** Closures are wonderfully useful, but tough to implement just right.

    A closure is created when an inner lambda or inner definition needs to
    access an outer value. That outer value can be from another definition, or
    from another lambda. Calls that use upvalues need to be closures, instead of
    typical definitions (o_create_function makes the copy).

    The values inside of a closure need to be fresh (current). To do that, the
    interpreter does shadowing: Before a read, the closure is pulled. After a
    write, the closure is written to. Shadowing is done by closure
    transformation, when a definition or lambda is done (see lily_closure.c).

    Some decisions have been made with closures in mind:

    * Class constructors are blocked from being a closure. This was implemented,
      but later removed. It was found to have sharp edge cases, and was not that
      useful.

    * Vars of a generic type cannot be closed over. Treating the A of an inner
      scope as the A of an outer scope is unsound.

    * Closures may supply the self for a method. This allows lambdas in a method
      to call other methods. **/

/* A function that uses upvalues is going to be used in some way. Make a copy of
   it into the storage given. */
...
/* Ensure that the var is in closure, or SyntaxError. Returns the closure spot
   of the var. */
