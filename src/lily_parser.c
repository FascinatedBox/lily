...
lily_state *lily_new_state(lily_config *config)
{
    ...
    /* Won't move and frequently used, so save a copy in parser too. */
    parser->prelude = parser->ims->prelude;
    ...
    parser->symtab = lily_new_symtab(parser->prelude);
...
void lily_free_state(lily_state *vm)
{
    ...
    lily_free_import_state(parser->ims);
...
/** When the interpreter loads a foreign module, it doesn't immediately load all
    symbols inside of it. Instead, the module provides the interpreter with two
    tables (which are parallel arrays). The info table specifies the kind of
    symbol, and the call table provides a loader. The tables are created by
    bindgen (not in this repo).

    This was originally designed to save memory: The interpreter can use type
    information and dynamically load only the symbols that are used.

    The first record is a header specifying how many classes and enums have been
    loaded. The foreign module (prelude aside), doesn't know what ids it will
    need. So they're loaded into a cid table, and bindgen provides macros.

    After the header are the records. Each record is a letter, an offset to the
    next record, the symbol name, then the data. */
...
{
    return (lily_class *)try_toplevel_dynaload(parser, parser->prelude, name);
}
    ...
    if (parser->current_class) {
        ...
        if (sym) {
            if (sym->item_kind & ITEM_IS_VARLIKE) {
                ...
                lily_raise_syn(parser->raiser,
                        "%s is a variant (use %s.%s or .%s instead).",
                        name, self_cls->name, name, name);
        }
    }
